...
export function ManageShowtimes() {
  ...
  const soldCount = (showtimeId: string) =>
    bookings.filter((b) => b.showtimeId === showtimeId && b.status === 'confirmed').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card style={{ background: '#1c1b1b', border: '1px solid rgba(255,255,255,0.06)' }}>
        <Table dataSource={showtimes} columns={columns} rowKey="id" size="small" />
      </Card>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          {searchResults && searchResults.results.length > 0 && (
            <Form.Item label="Chọn phim" name="movieId" rules={[{ required: true, message: 'Chọn phim' }]}>
              <Select
                showSearch
                loading={isFetching}
                ...
                filterOption={false}
                onChange={(value) => {
                  const movie = searchResults.results.find((m) => m.id === value);
                  if (movie) {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Form.Item label="Giờ chiếu" name="time" rules={[{ required: true }]}>
              <TimePicker style={{ width: '100%' }} format="HH:mm" minuteStep={15} />
            </Form.Item>
          </div>

          <Form.Item label="Phòng chiếu" name="hallId" rules={[{ required: true }]}>
